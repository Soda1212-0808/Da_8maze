function results = detect_place_cell_v4(positionX, positionY, position_timelite, spike_times, inIntervals, bin_size, varargin)
...
%%% ADDED: v_thresh parameter (default 2)
addParameter(p,'v_thresh',5,@(x)isnumeric(x)&&isscalar(x)&&x>=0);            % speed threshold (units same as position/time units)
...
%%% ADDED: compute instantaneous speed and apply threshold to mask_edges/include_mask
% keep simple speed calculation: euclidean displacement / dt
dx = [0; diff(positionX)];
dy = [0; diff(positionY)];
dt_pos = [diff(position_timelite); median(diff(position_timelite))];
if any(dt_pos <= 0)
    % fallback median positive dt
    mdt = median(dt_pos(dt_pos>0));
    if isempty(mdt) || ~isfinite(mdt)
        mdt = 1;
    end
    dt_pos(dt_pos <= 0) = mdt;
end
speed = sqrt( (dx ./ dt_pos).^2 + (dy ./ dt_pos).^2 ); % units: position units / second

% mask_edges: include only samples with valid pos, in intervals, AND speed > v_thresh (running)
mask_edges = valid_pos & mask_any & (speed > v_thresh);
if ~any(mask_edges)
...
% include_mask: only those samples with valid pos AND in any interval AND speed>v_thresh / 仅包含有效且在区段内且为跑动的样本
include_mask = valid_pos & mask_any & (speed > v_thresh);
