...
for curr_animal=1:length(animals)
    ...
    %% 保存轨迹 BW1
    bufferfolderName = 'bufferFile';
    X_position=cellfun(@(x)table2array(x(:,2))+100,sort_data_path,'UniformOutput',false);
    Y_position=cellfun(@(x)table2array(x(:,3))+100,sort_data_path,'UniformOutput',false);
    ...
    all_data_frame = cellfun(@(A,B,C) [B(:,1:3) round(interp1(double(cell2mat(A.animal_match(C, 8))), cell2mat(A.animal_match(C, 7)),B(:,4:11), 'linear', 'extrap')) ],...
        ...
        for i=1:2
            ...
            for curr_trial=1:size(all_data_frame{i},1)
                x_b{i}{curr_trial}=all_data_X_filter_speed{i}(all_data_frame{i}(curr_trial,8):all_data_frame{i}(curr_trial,10));
                y_b{i}{curr_trial}=all_data_Y_filter_speed{i}(all_data_frame{i}(curr_trial,8):all_data_frame{i}(curr_trial,10));
...
%   event_used_idx(2:end-1);
    response_t = response_t_list{win_idx};  % 取当前 response 窗口
    ...
    event_response_all{curr_day}{curr_timepoint}{win_idx}{curr_arm} = event_response;
    ...
    event_response_p_all{curr_day}{curr_timepoint}{win_idx}{curr_arm} = event_response_rank(:,1) ./ (n_shuff + 1);
end
end
end
end
...
event_response_p_all_5=permute(max(event_response_p_all_4,[],3),[1 2 4 5 3]);
...
for i=1:size(event_response_p_all_5,1)
    ...
    for j=1:7
        nexttile
    imagesc(permute(event_response_p_all_5(i,j,:,:)>0.95,[3 4 1 2]))
        title(num2str(j))
...
significant_windows = find(cellfun(@(p) any(p < alpha_corrected), event_response_p_all));
        ...
        for curr_cell=1: size(all_data_imgaing{curr_day},1)
            ...
            for curr_day=1:length(all_data_frame)
                ...
                spike_x{curr_day} = all_data_X_filter_speed{curr_day}(intersect(find(spike_times>20),[sample_phase_segments{:}]'));
                spike_y{curr_day} = all_data_Y_filter_speed{curr_day}(intersect(find(spike_times>20),[sample_phase_segments{:}]'));
