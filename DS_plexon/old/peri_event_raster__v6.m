...
for curr_animal=1:length(animals)
    ...
    for curr_file=1:2
        ...
        event_times{3}=data_event_sorted(:,5);
        event_times{4}=data_event_sorted(:,6);
        event_times{5}=data_event_sorted(:,7);
        event_times{6}=data_event_sorted(:,8);
        event_times{7}=data_event_sorted(:,9);
        event_times{8}=data_event_sorted(:,10);
        ...
        for curr_cell=1:length(spikes_all)
            ...
            figure_name={'sample begin','sample arm','sample reward','delay','choice begin','choice arm','sample reward','end'}
            % 绘制 raster 图
...
%                 scatter(valid_spikes{curr_figure}, trial_indices{curr_figure}, 3,[0.5 0.5 0.5], 'filled');
                    [~, first_indices] = unique(data_event_sorted(:,1), 'first');
                    hold on
