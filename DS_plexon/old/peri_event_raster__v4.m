...
for curr_animal=1:length(animals)
    ...
    for curr_file=1:2
        ...
        event_times=data_event_sorted(:,7);
        ...
        for curr_cell=1:length(spike_whole)
            ...
            % 为每种event创建逻辑索引
            idx_{2} = ismember(data_event_sorted(:,1), [26, 62]);
            idx_{1} = ismember(data_event_sorted(:,1), [17, 71]);
            idx_{3} = ismember(data_event_sorted(:,1), [35, 53]);
